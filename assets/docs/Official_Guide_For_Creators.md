...
## 12. クリエイター（およびAI）が必ず陥る罠
エフェクトの開発や配布において、**多くの初心者（やAIアシスタント）が陥る罠**がいくつか存在します。

### 罠1：キーボード入力とシステム競合（操作の競合）
インタラクティブなエフェクトを作ろうとして、キャラクターの移動などに `WASD` や矢印キーを使おうとするのは危険です。

> [!CAUTION]
> **エフェクト内の主要な操作にキーボード入力を使用しないでください！**

ScenePlus+ では、**キーボードのキーがエフェクト発火のトリガーとしてシステム側にフックされる仕様**となっています。そのため、エフェクトの内部処理でキー入力を自由に使用すると、システムの発火トリガーと競合してしまいます。

**【代替案】**
インタラクティブな操作には、以下のようなマウス入力への代替を強く推奨します。
- `env.mouseState`（クリック状態や座標）
- `window.api.onMouseWheel`（マウスホイール操作）

### 罠2：パッケージング（フォルダ圧縮のミス）
完成したエフェクトを配布可能な形式（`.zip` または `.scenefx`）にする際、99%の初心者が陥る罠です。
...
### 罠3：AIによるコード生成時の出力形式
AIアシスタントを用いてコードを生成させる際（またはAI自身がこのドキュメントを読んでいる場合）の注意点です。

> [!CAUTION]
> **エフェクトを作成するAIは、ZIPファイルではなくコードブロック形式で出力してください**

AIがエフェクトを作成する際、完成品を `.zip` ファイルとして出力しようとする場合がありますが、これは推奨されません。
生成されたコードはユーザーが確認しやすいよう、必ず**マークダウンのコードブロック形式**で `meta.json` や `.js` の中身を出力するようにしてください。
